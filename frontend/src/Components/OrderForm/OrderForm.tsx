...
import ModalOrderDetailes from './components/ModalOrderDetailes/ModalOrderDetailes'
...
import { classNames, extractToken } from '../../utils/utils'
import { createOrderArray, navigation, teams } from './OrderForm.utiles'
...
const OrderForm = ({fetchurl}:OrderFormProps) => {
  ...
  const fetchOrders = async () =>{
    try {
      ...
      const {data} = await axios.get(
          `${URLS.BACKEND_URL}/${fetchurl}`,
          config
      )

      const myOrders = createOrderArray(data)
      setOrders(myOrders)
  ...
  const showModalOrders = (order:Order) => {
          ...
          <main className="flex-1">
            ...
            <div className="border-b border-gray-200 px-4 py-4 sm:flex sm:items-center sm:justify-between sm:px-6 lg:px-8">
              <div className="flex-1 min-w-0">
                <h1 className="text-lg font-medium leading-6 text-gray-900 sm:truncate">My Order</h1>
              </div>
            </div>
                  ...
                  <thead>
                    <tr className="border-t border-gray-200">
                      <th
                        ...
                        scope="col"
                      >
                        <span className="lg:pl-2">Order Number</span>
                      </th>
                      <th
                        className="px-6 py-3 border-b border-gray-200 bg-gray-50 text-left text-xs font-medium text-gray-500 uppercase tracking-wider"
                        scope="col"

...
import { classNames, extractToken, isAdmin } from '../../utils/utils'
...
const AllProductScreen = () => {
  ...
  const [prodectRemove,setProdectRemove] = useState<Product | null>(null)
  const [isExcecuteRemove,setIsExcecuteRemove] = useState(false)
  ...
  const handleRemove = async (id:string | undefined)=>{
    setIsExcecuteRemove(true)
    try{
      const config = extractToken()
      await axios.delete(`${URLS.BACKEND_URL}/${URLS.PRODUCTS}/${id}`,config)
      fetchProducts()
    }catch(error:any){
       console.log(error)
    }finally{
      setIsExcecuteRemove(false)
    }
  }

  const onAccapt =(id:string | undefined)=>{ 
    handleRemove(id)
    changeOpen(false) 
  }
    ...
    products?.map((product) => (
      <tr key={product.id}>
        ...
        <td className="px-6 py-3 whitespace-nowrap text-right text-sm font-medium">
          <Link to={`${ROUTES.UPDATE_PRODUCT}/${product.id}`}>
              <span 
    ...
    <>
      ...
      </div>
      {
        ...
        <ModalRemoveProduct 
            ...
            id={prodectRemove?.id.toString()}
            name={prodectRemove?.name}
            open={open}
            onAccapt={onAccapt}
        />
